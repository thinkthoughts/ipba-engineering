...
def validate_context(context: RepositoryContext) -> None:
    ...
    scalar_fields = {
        ...
        "engineering_statement_title": context.engineering_statement_title,
        ...
        "repository_sequence_title": context.repository_sequence_title,
        "repository_sequence_caption": context.repository_sequence_caption,
    ...
    for name, value in scalar_fields.items():
        ...
        if not value.strip():
            raise ContextValidationError(f"{name} must be non-empty")
    ...
    for name, values in sequence_fields.items():
        ...
        if any(not isinstance(item, str) for item in values):
            raise ContextValidationError(f"{name} must contain only strings")
        if any(not item.strip() for item in values):
            raise ContextValidationError(f"{name} contains an empty item")
    ...
    expected_relationship_count = len(context.lane_symbols) - 1
    if len(context.lane_relationships) != expected_relationship_count:
        raise ContextValidationError(
    ...
    if not context.construction_sequence[0].startswith("00 "):
        raise ContextValidationError(
    ...
    for item in context.construction_sequence:
        ...
        if not number.isdigit():
            raise ContextValidationError(
                f"invalid notebook number in construction_sequence: {number!r}"
            )
        if not title.strip():
            raise ContextValidationError(
                f"construction_sequence item {item!r} must include a title"
            )
    ...
    if not context.next_notebook.startswith(f"{next_number}_"):
        raise ContextValidationError(
            "next_notebook must match the second item in construction_sequence"
        )

    if not context.next_notebook.endswith(".ipynb"):
        raise ContextValidationError(
